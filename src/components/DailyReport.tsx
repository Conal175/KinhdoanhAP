...
import { Calendar, Plus, Edit2, Trash2, ChevronDown, ChevronRight, BarChart3, DollarSign, Eye, MessageCircle, ShoppingCart, Receipt } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h2 className="text-2xl font-bold flex items-center gap-2"><BarChart3 className="w-7 h-7 text-indigo-600" /> Báo Cáo & Nhật Ký Vận Hành</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-white">
        <div className="bg-emerald-600 p-4 rounded-xl shadow-md font-bold"><p className="text-xs opacity-80">Doanh thu</p><p className="text-xl">{summary.revenue.toLocaleString()}đ</p></div>
        <div className="bg-blue-600 p-4 rounded-xl shadow-md font-bold"><p className="text-xs opacity-80">Chi phí QC</p><p className="text-xl">{summary.spend.toLocaleString()}đ</p></div>
        <div className="bg-amber-500 p-4 rounded-xl shadow-md font-bold"><p className="text-xs opacity-80">CPO</p><p className="text-xl">{summary.orders > 0 ? Math.round(summary.spend/summary.orders).toLocaleString() : 0}đ</p></div>
        <div className="bg-indigo-600 p-4 rounded-xl shadow-md font-bold"><p className="text-xs opacity-80">Đơn hàng</p><p className="text-xl">{summary.orders}</p></div>
      </div>
          ...
          return (
            <div key={day} className={`bg-white rounded-xl border border-gray-200 overflow-hidden ${day===new Date().getDate()?'ring-2 ring-indigo-500':''}`}>
              <div onClick={() => setExpandedDays(isExpanded ? expandedDays.filter(d => d!==day) : [...expandedDays, day])} className="p-3 flex justify-between items-center cursor-pointer hover:bg-gray-50 font-bold">
                 <span className="flex items-center gap-2 text-gray-700">{isExpanded ? <ChevronDown className="w-4 h-4"/> : <ChevronRight className="w-4 h-4"/>} Ngày {day} ({dayLogs.length} QC)</span>
                 {canEdit && <button onClick={(e) => { e.stopPropagation(); setIsAdding(day); setEditingLog(null); setFormData({adName:'', spend:0, orders:0, revenue:0}); }} className="p-1 bg-indigo-600 text-white rounded"><Plus className="w-4 h-4"/></button>}
              </div>
              {isExpanded && (
                <div className="border-t overflow-x-auto">
                   ...
                   <table className="w-full text-xs text-left">
                      <thead className="bg-gray-50"><tr><th className="p-2">Bài QC</th><th className="p-2 text-right">Chi phí</th><th className="p-2 text-center">Đơn</th><th className="p-2 text-right">Doanh thu</th><th className="p-2 text-center">#</th></tr></thead>
                      <tbody>
                        {dayLogs.map(log => (
                          <tr key={log.id} className="border-t">
                            <td className="p-2 font-medium">{log.adName}</td>
                            <td className="p-2 text-right text-blue-600 font-bold">{log.spend.toLocaleString()}đ</td>
                            <td className="p-2 text-center">{log.orders}</td>
                            <td className="p-2 text-right text-emerald-600 font-bold">{log.revenue.toLocaleString()}đ</td>
                            <td className="p-2 text-center">
                               <div className="flex justify-center gap-1">
                                  {canEdit && <button onClick={() => { setEditingLog(log); setFormData({...log}); }} className="text-amber-600 p-1"><Edit2 className="w-4 h-4"/></button>}
                                  {canDelete && <button onClick={() => { if(confirm('Xóa?')) saveLogs(logs.filter(l => l.id !== log.id)); }} className="text-red-500 p-1"><Trash2 className="w-4 h-4"/></button>}
                               </div>
                            </td>
                          </tr>
                        ))}
