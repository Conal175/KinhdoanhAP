...
const DailyReport: React.FC<DailyReportProps> = ({ projectId }) => {
  ...
  useEffect(() => {
    setLogs(getDailyLogs(projectId));
  }, [projectId]);

  const saveLogsToStorage = (newLogs: DailyLog[]) => {
    saveDailyLogs(projectId, newLogs);
    setLogs(newLogs);
  };
  ...
  const getDaySummary = (dayLogs: DailyLog[]) => {
    ...
    return {
      ...
      cpa: calculateCPA(totalSpend, totalMessages),
      cpo: calculateCPO(totalSpend, totalOrders),
      cr: calculateCR(totalOrders, totalMessages)
    };
  };

  const getMonthSummary = () => {
    const monthLogs = logs.filter(l => l.month === selectedMonth && l.year === selectedYear);
    return getDaySummary(monthLogs);
  };
  ...
  const daysWithLogs = logs
    .filter(l => l.month === selectedMonth && l.year === selectedYear)
    .map(l => l.day);
  const uniqueDaysCount = new Set(daysWithLogs).size;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="lg:col-span-2">
          ...
          <input
            ...
            placeholder="VD: Camp 1 - Video máy bơm"
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-${colorScheme}-500`}
          />
        </div>
        <div className="lg:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <DollarSign className="w-3 h-3 inline" /> Ngân sách (Spend)
          </label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <Eye className="w-3 h-3 inline" /> Lượt hiển thị
          </label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <MousePointer className="w-3 h-3 inline" /> Lượt nhấp (Clicks)
          </label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <MessageCircle className="w-3 h-3 inline" /> Tin nhắn mới
          </label>
          <input
            type="number"
            value={formData.messages || ''}
            onChange={(e) => setFormData({...formData, messages: Number(e.target.value)})}
            placeholder="0"
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-${colorScheme}-500`}
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <ShoppingCart className="w-3 h-3 inline" /> Đơn hàng
          </label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <Receipt className="w-3 h-3 inline" /> Doanh thu (VNĐ)
          </label>
          <input
            type="number"
        ...
        <div className="lg:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <AlertTriangle className="w-3 h-3 inline text-red-500" /> Vấn đề phát sinh
          </label>
          <textarea
            value={formData.issues}
            onChange={(e) => setFormData({...formData, issues: e.target.value})}
            rows={2}
            placeholder="VD: Chạy đắt, khách boom hàng..."
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-${colorScheme}-500`}
          />
        </div>
        <div className="lg:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            <Zap className="w-3 h-3 inline text-green-500" /> Hành động tối ưu
          </label>
          <textarea
            value={formData.optimizations}
            onChange={(e) => setFormData({...formData, optimizations: e.target.value})}
            rows={2}
            placeholder="VD: Đã tắt nhóm tuổi 18-24..."
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-${colorScheme}-500`}
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div>
            ...
            <span className="ml-2 font-semibold text-purple-600">
              {formatPercent(calculateCTR(formData.clicks, formData.impressions))}
            </span>
          </div>
          <div>
            <span className="text-gray-500">CPA:</span>
            <span className="ml-2 font-semibold text-blue-600">
              {formatCurrency(calculateCPA(formData.spend, formData.messages))}đ
            </span>
          </div>
          <div>
            <span className="text-gray-500">CPO:</span>
            <span className="ml-2 font-semibold text-amber-600">
              {formatCurrency(calculateCPO(formData.spend, formData.orders))}đ
            </span>
          </div>
          <div>
            <span className="text-gray-500">Tỷ lệ chốt:</span>
            <span className="ml-2 font-semibold text-green-600">
              {formatPercent(calculateCR(formData.orders, formData.messages))}
            </span>
          </div>
        </div>
      </div>
    ...
    if (log.adLink) {
      return (
        <a
          ...
          className="font-medium text-indigo-600 hover:text-indigo-800 hover:underline inline-flex items-center gap-1 group"
          title={log.adLink}
        >
          {log.adName}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Month Summary Cards - Row 1: Doanh số, Chi phí, CPO first */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl p-4 text-white">
          ...
          <div className="text-xl font-bold mt-1">{formatCurrency(monthSummary.revenue)}đ</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl p-4 text-white">
          ...
          <div className="text-xl font-bold mt-1">{formatCurrency(monthSummary.cpo)}đ</div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-white rounded-xl p-4 border shadow-sm">
          ...
          <div className="text-xl font-bold text-cyan-600 mt-1">{formatCurrency(monthSummary.clicks)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 border shadow-sm">
          <div className="flex items-center gap-2 text-gray-500 text-sm">
          ...
          <div className="text-xl font-bold text-blue-600 mt-1">{formatCurrency(monthSummary.messages)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 border shadow-sm">
          <div className="text-gray-500 text-sm">CTR</div>
          <div className="text-xl font-bold text-purple-600 mt-1">{formatPercent(monthSummary.ctr)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 border shadow-sm">
          <div className="text-gray-500 text-sm">CPA</div>
          <div className="text-xl font-bold text-blue-600 mt-1">{formatCurrency(monthSummary.cpa)}đ</div>
        </div>
        <div className="bg-white rounded-xl p-4 border shadow-sm">
          <div className="text-gray-500 text-sm">Tỷ lệ chốt</div>
          <div className="text-xl font-bold text-green-600 mt-1">{formatPercent(monthSummary.cr)}</div>
        </div>
      </div>
            ...
            <div key={day} className={`bg-white rounded-xl border overflow-hidden ${isToday(day) ? 'ring-2 ring-indigo-500' : ''}`}>
              ...
              <div
                ...
                className={`flex items-center justify-between px-4 py-3 cursor-pointer transition-colors ${
                  hasLogs ? 'bg-gradient-to-r from-indigo-50 to-white hover:from-indigo-100' : 'hover:bg-gray-50'
                }`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`font-semibold ${isToday(day) ? 'text-indigo-600' : 'text-gray-700'}`}>
                      Ngày {day}
                    </span>
                    {isToday(day) && (
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleAddLog(day); }}
                  className="p-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors ml-2 shrink-0"
                  title="Thêm báo cáo"
                >
                  ...
                  {dayLogs.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {dayLogs.map((log, idx) => (
                            <tr key={log.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-indigo-50/50 transition-colors`}>
                              <td className="px-3 py-2.5 min-w-[180px]">
                                <div className="flex flex-col">
                                  {renderAdName(log)}
                                  {log.adLink && (
                                    <span className="text-xs text-gray-400 mt-0.5 truncate max-w-[200px]" title={log.adLink}>
                                      🔗 {(() => { try { return new URL(log.adLink).hostname; } catch { return log.adLink; } })()}
                                    </span>
                                  )}
                                </div>
                              </td>
                              <td className="px-3 py-2.5 text-right text-blue-600 font-medium whitespace-nowrap">{formatCurrency(log.spend)}đ</td>
                              <td className="px-3 py-2.5 text-right text-purple-600 whitespace-nowrap">{formatCurrency(log.impressions)}</td>
                              <td className="px-3 py-2.5 text-right text-cyan-600 whitespace-nowrap">{formatCurrency(log.clicks)}</td>
                              <td className="px-3 py-2.5 text-right text-amber-600 whitespace-nowrap">{log.messages}</td>
                              <td className="px-3 py-2.5 text-right text-green-600 font-medium whitespace-nowrap">{log.orders}</td>
                              <td className="px-3 py-2.5 text-right text-emerald-600 font-medium whitespace-nowrap">{formatCurrency(log.revenue)}đ</td>
                              <td className="px-3 py-2.5 text-right text-purple-600 whitespace-nowrap">{formatPercent(calculateCTR(log.clicks, log.impressions))}</td>
                              <td className="px-3 py-2.5 text-right text-blue-600 whitespace-nowrap">{formatCurrency(calculateCPA(log.spend, log.messages))}đ</td>
                              <td className="px-3 py-2.5 text-right text-amber-600 whitespace-nowrap">{formatCurrency(calculateCPO(log.spend, log.orders))}đ</td>
                              <td className="px-3 py-2.5 text-right text-green-600 font-medium whitespace-nowrap">{formatPercent(calculateCR(log.orders, log.messages))}</td>
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-6 text-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          <div>
            <div className="text-indigo-200 text-sm">Ngày có báo cáo</div>
            <div className="text-2xl font-bold">{uniqueDaysCount} ngày</div>
          </div>
          <div>
            <div className="text-indigo-200 text-sm">Doanh thu</div>
            <div className="text-2xl font-bold">{formatCurrency(monthSummary.revenue)}đ</div>
          </div>
          <div>
            <div className="text-indigo-200 text-sm">Tổng chi tiêu</div>
            <div className="text-2xl font-bold">{formatCurrency(monthSummary.spend)}đ</div>
          </div>
          <div>
            <div className="text-indigo-200 text-sm">CPO</div>
            <div className="text-2xl font-bold">{formatCurrency(monthSummary.cpo)}đ</div>
          </div>
          <div>
            <div className="text-indigo-200 text-sm">Tổng đơn hàng</div>
            <div className="text-2xl font-bold">{monthSummary.orders} đơn</div>
          </div>
          <div>
            <div className="text-indigo-200 text-sm">ROAS</div>
            <div className="text-2xl font-bold">
              {monthSummary.spend > 0 ? (monthSummary.revenue / monthSummary.spend).toFixed(2) : '0'}x
            </div>
          </div>
        </div>
      </div>
